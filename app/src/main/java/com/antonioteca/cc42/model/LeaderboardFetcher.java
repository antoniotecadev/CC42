...
public class LeaderboardFetcher {
    ...
    public interface LeaderboardCallback {
        void onLeaderboardFetched(List<UserScore> topUsers, int numberSubscriptions);

        void onError(String errorMessage);
    }

    public void fetchTopUsersForMeal(Integer campusId, Integer cursusId, String mealId, LeaderboardCallback callback) {
        if (mealId == null || mealId.isEmpty()) {
            callback.onError("Meal ID não pode ser nulo ou vazio.");
            return;
        }

        DatabaseReference mealsRef = databaseReference.child("campus").child(String.valueOf(campusId))
                .child("cursus")
        ...
        mealsRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    Integer quantityMeal = dataSnapshot.child("quantity").getValue(Integer.class);
                    Integer numberSubscriptions = (int) dataSnapshot.child("subscriptions").getChildrenCount();
                    if (quantityMeal != null) {

                        int numberOfMealsAvailable = quantityMeal - numberSubscriptions;

                        if (numberOfMealsAvailable <= 0) {
                            callback.onError("Número de refeições esgotadas.");
                            return;
                        }
                        ...
                        query.addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                ...
                                if (dataSnapshot.exists()) {
                                    ...
                                    callback.onLeaderboardFetched(topUsers, numberOfMealsAvailable);

                                } else {
                                    callback.onLeaderboardFetched(new ArrayList<>(), numberOfMealsAvailable); // Nenhum usuário encontrado
                                }
                            }
                        ...
                        callback.onError("Quantidade de refeições não encontrada.");
                } else {
                    callback.onError("Refeição não encontrada.");
                }
            }
